...
export const SubmitButton = ({ buttonState }) => {
    const { isPostLoading, isPostSuccess, isFormError, isFormLoading } = buttonState;
    return (
        <span className={`w-full text-white p-2 rounded-lg flex justify-center
            ${isPostLoading || isPostSuccess || isFormError || isFormLoading ? "bg-slate-400 cursor-not-allowed" : 
        ...
        {isFormError ? (
            <div classname='flex'>
                ...
                isPostSuccess ? <ConfirmIcon size={25}/> : (
                    <input disabled={isFormLoading} type="submit" value="Save" className='cursor-pointer w-full'/>
                )
        }
        </span>
    );
