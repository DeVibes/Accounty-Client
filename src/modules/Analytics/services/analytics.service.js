...
export const mapDataForBalance = (data) =>
    data
        .map(({ Category: name, Total: value }) => {
            const color = CategoriesMap.get(name)?.color;
...
export const calculateTotalSpentAndIncome = (transactions) => {
    let total = 0,
        income = 0;
    if (!transactions) return { total, income };
    transactions.forEach((tr) => {
        if (tr.category.name !== Category.Income.name) total -= tr.price;
        else income += tr.price;
    });
    return { total, income };
};
...
export const getTimeFrame = () => {
    ...
    if (currentDay > userCycle) {
        ...
        fromDate = DateTime.local(currentYear, currentMonth, userCycle + 1)
            .startOf('day')
            .toUTC()
            .toISO(); //'YYYY-MM-DDTHH:MM:SS.MSZ'
