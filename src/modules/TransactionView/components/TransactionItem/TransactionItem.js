...
export const TransactionItem = ({ data, isAbsoluteFirst }) => {
    const { selectedTransaction, updateSelectedTransaction } =
    ...
    const Icon =
        CategoriesMap.get(data.category)?.icon ?? Category.Unknown.icon;
    const isBottomBorderShown =
        (!data.isFirst || isAbsoluteFirst) && !isSelected;
    ...
    const handleDeleteClick = (e) => {
        e.stopPropagation();
        if (isConfirmDelete) deleteTransaction(data.id);
    ...
    return (
        <>
            ...
            <div
                className={`flex justify-between rounded-xl overflow-hidden ${
            ...
            >
                ...
                >
                    <TransactionLeftActions
                        isTransactionSeen={isTransactionSeen}
                ...
                <SlideAnimationHOC
                    trigger={isSelected}
                    elementWidth={isConfirmDelete ? 80 : 57}
                    direction="left"
                    className="flex"
                >
                    <ExpendAnimationHOC
                        trigger={isConfirmDelete}
                        className={`flex`}
                    >
                        <TransactionRightData
                            price={data.price}
                            paymentName={data.paymentType}
                        />
                        <TransactionRightActions
