...
export const FilterForm = ({ handleClose }) => {
  ...
  const onUpdateClick = data => {
    ...
    setDateRange(formattedDates.fromDate, formattedDates.endDate)
    handleClose();
  };
  ...
  return (
    <form onSubmit={onSubmit} className='grid grid-rows-2 grid-cols-2 gap-4'>
      <div className="">
				<Input name="fromDate" type='datetime-local' register={register} required label="Start Date"/>
				{errors.fromDate && <ErrorMsg msg="Please enter start date"/>}
      </div>
      <div className="">
				<Input name="toDate" type='datetime-local' register={register} required label="To Date"/>
				{errors.toDate && <ErrorMsg msg="Please enter end date"/>}
      </div>
			<div className="col-span-2 mt-2">
        <button type='submit' className="rounded-xl bg-[#7aa2f7] w-full h-full">Update</button>
			</div>
    </form>
  );
};
